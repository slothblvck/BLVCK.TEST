...
const WinnerCardAuction = ({ winner, userInfo }) => {
    ...
    return (
        <div className='w-full h-44 bg-[#101010] rounded-lg drop-shadow-[4px_4px_16px_rgba(0,0,0,0.16)] flex items-center justify-center relative overflow-hidden'>
            ...
            <h1 className='absolute h-full w-full flex items-center justify-center rounded-lg text-2xl font-[500] top-3'>
                {winner[curr]?.username || winner[curr]?.walletAddress?.substr(0, 5) + '...' +winner[curr]?.walletAddress?.substr(winner[curr]?.walletAddress?.length-5, winner[curr]?.walletAddress?.length)}
            </h1>
            {userInfo.isAdmin && (
...
export default WinnerCardAuction
